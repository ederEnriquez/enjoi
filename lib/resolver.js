...
class SchemaResolver {
    ...
    number(schema) {
        ...
        schema.inclusiveLess && (joischema = joischema.min(Joi.ref(schema.inclusiveLess)));
        schema.inclusiveGreater && (joischema = joischema.max(Joi.ref(schema.inclusiveGreater)));
        ...
        if(schema.operation){
            const custom = Joi.extend((joi) => {
                return {
                    ...
                    base: joi.number(),
                    messages: {
                        'operation.sum': 'The sum of {{#tags}} must be equal to {{#result}}',
                        'operation.big': '"{{#label}}" must be at least five millions',
                        'operation.round': '"{{#label}}" must be a round number',
                        'operation.dividable': '"{{#label}}" must be dividable by {{#q}}'
                    },
                    rules: {
                        sum: {
                            alias: 'sum',
                            method(tags, result) {
                                const config = {tags, result};
                                return this.$_addRule({ name: 'sum', args: { config } });
                            },
                            validate(value, helpers, args) {
                                const config = args.config;
                                if(helpers.state.ancestors){
                                    const values = helpers.state.ancestors[0];
                                    const sum = config.tags.reduce((a, b) => +a + +values[b], 0);
                                    if(config.result !== sum)
                                        return helpers.error('operation.sum', { result: config.result, tags: config.tags.join(", ")  });
                                }
                            }
                        }
                    }
            ...
            joischema = custom.operation()[schema.operation.type](schema.operation.refs, schema.operation.result);
        }
        return joischema;
    }
